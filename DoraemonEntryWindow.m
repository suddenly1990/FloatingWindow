...
- (UIButton *)entryBtn {
    if (!_entryBtn) {
        ...
        _entryBtn.backgroundColor = [UIColor purpleColor];
        UIImage *image = GET_IMAGE(@"doraemon_logo");
#if defined(__IPHONE_13_0) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0)
        if (@available(iOS 13.0, *)) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                image = GET_IMAGE(@"doraemon_logo_dark");
            }
        }
#endif
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection {
    ...
    if (@available(iOS 13.0, *)) {
        if ([self.traitCollection hasDifferentColorAppearanceComparedToTraitCollection:previousTraitCollection]) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
//                [self.entryBtn setImage:GET_IMAGE(@"doraemon_logo_dark") forState:UIControlStateNormal];
            } else {
//                [self.entryBtn setImage:GET_IMAGE(@"doraemon_logo") forState:UIControlStateNormal];
            }
        }
    }
#endif
...
- (void)showClose:(NSNotification *)notification{
    [_entryBtn setImage:GET_IMAGE(@"doraemon_close") forState:UIControlStateNormal];
...
- (void)closePluginClick:(UIButton *)btn{
    [_entryBtn setImage:GET_IMAGE(@"doraemon_logo") forState:UIControlStateNormal];
...
//        DoKitBP(@"dokit_sdk_home_ck_entry") // 埋点管理
    }else{
